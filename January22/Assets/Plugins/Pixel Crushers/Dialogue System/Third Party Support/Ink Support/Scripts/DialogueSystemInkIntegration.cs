...
namespace PixelCrushers.DialogueSystem.InkSupport
{
    ...
    public class DialogueSystemInkIntegration : MonoBehaviour
    {
        ...
        [Tooltip("If ticked, match actor names case sensitive. If unticked, do case-insensitive matching.")]
        public bool caseSensitiveActorNames = false;
        ...
        protected bool wasDatabaseLoadedIntoMemory = false;
        protected Dictionary<string, Story> runtimeAddedStoryDict = new Dictionary<string, Story>();
        ...
        protected virtual void CreateDatabase()
        {
            ...
            wasDatabaseLoadedIntoMemory = true;
        }
        ...
        public virtual void AddStory(string storyTitle, string storyJSON)
        {
            ...
            if (wasDatabaseLoadedIntoMemory)
            {
                runtimeAddedStoryDict.Add(storyTitle, story);
            }
            nextStoryConversationID++;
        }
        ...
        protected virtual void AddStoryToDatabase(string storyTitle, Story story, int conversationID)
        {
            ...
            foreach (var variableName in variables)
            {
                ...
                if (wasDatabaseLoadedIntoMemory)
                {
                    DialogueManager.masterDatabase.variables.Add(variable);
                }
            }

            // If database was already loaded into memory, we need to add this new story into masterDatabase manually:
            if (wasDatabaseLoadedIntoMemory)
            {
                DialogueManager.masterDatabase.AddConversation(inkConversation);
                var conversationTable = Lua.Environment.GetValue("Conversation") as Language.Lua.LuaTable; // Note: LuaInterpreter
                DialogueLua.AddToConversationTable(conversationTable, inkConversation);
                PersistentDataManager.InitializeNewVariablesFromDatabase();
            }
        ...
        public static Story LookupStory(string storyName)
        {
            ...
            foreach (var kvp in instance.runtimeAddedStoryDict)
            {
                if (string.Equals(kvp.Key, storyName))
                {
                    return kvp.Value;
                }
            }
        ...
        {
            if (!storyDict.ContainsKey(DialogueManager.lastConversationID)) return;
            if (disableAlwaysForceResponseMenu)
            {
        ...
        protected virtual void OnPrepareConversationLine(DialogueEntry entry)
        {
            ...
            if (entry.id == 1)
            {
                ...
                if (!activeStory.canContinue)
                {
                    ...
                    {
                        entry.DialogueText = ProcessRichText(activeStory.currentText);
                ...
                entry.DialogueText = ProcessRichText(text);
        ...
        protected virtual string ProcessRichText(string s)
        {
            return s.Replace("<br>", "\n");
        }
        ...
        protected virtual void AddChoices(Story activeStory, Conversation inkConversation, DialogueEntry entry)
        {
            entry.outgoingLinks.Clear();
            inkConversation.dialogueEntries.RemoveAll(x => x.id >= 2);
            //var hasPlayerResponse = false;
            for (int i = 0; i < activeStory.currentChoices.Count; i++)
            {
                ...
                {
                    TryExtractPrependedActor(ref choiceText, responseEntry);
                }
                if (activeStory.currentChoices.Count == 1 && forceResponseMenuForSingleChoices)
                {
                    choiceText = "[f]" + choiceText;
                }
                responseEntry.MenuText = ProcessRichText(choiceText);
                responseEntry.Sequence = playerDialogueTextFollowsResponseText ? "Continue()" : string.Empty; // Will be shown in loopback to entry 1 (unless silent).
        ...
        protected virtual void TryExtractPrependedActor(ref string text, DialogueEntry entry)
        {
            if (text != null && text.Contains(":"))
            {
                ...
                if (colonPos < text.Length - 1)
                {
                    ...
                    var actor = caseSensitiveActorNames
                        ? DialogueManager.masterDatabase.GetActor(actorName)
                        : FindActorCaseInsensitive(actorName);
                    if (actor != null)
                    {
        ...
        protected virtual Actor FindActorCaseInsensitive(string actorName)
        {
            foreach (var actor in DialogueManager.masterDatabase.actors)
            {
                if (string.Equals(actorName, actor.Name, System.StringComparison.OrdinalIgnoreCase))
                {
                    return actor;
                }
            }
        ...
        IEnumerator PlaySequenceAtEndOfFrame(string sequence)
        {
            ...
            {
                speaker = DialogueManager.currentConversationState.subtitle.speakerInfo.transform;
                ...
                sequencer.subtitleEndTime = DialogueManager.conversationView.GetDefaultSubtitleDuration(subtitle.formattedText.text);
                sequence = sequence.Replace("{{default}}", DialogueManager.conversationView.GetDefaultSequence(subtitle));
            }
            sequence = sequence.Replace(SequencerKeywords.End, sequencer.subtitleEndTime.ToString(System.Globalization.CultureInfo.InvariantCulture));
            sequencer.SetParticipants(speaker, listener);
        ...
        protected virtual void RegisterLuaFunctions()
        {
            ...
            Lua.RegisterFunction("SetInkList", null, SymbolExtensions.GetMethodInfo(() => SetInkList(string.Empty, string.Empty)));
            ...
            Lua.RegisterFunction("GetInkList", null, SymbolExtensions.GetMethodInfo(() => GetInkList(string.Empty)));
        }
        ...
        public static string GetInkList(string variableName)
        {
            if (m_instance == null) return string.Empty;
            var value = m_instance.GetInkVariableValue(variableName);
            return (value != null) ? value.ToString() : string.Empty;
